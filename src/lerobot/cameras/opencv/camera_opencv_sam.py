...
class OpenCVCamera(Camera):
    ...
    def find_cameras() -> list[dict[str, Any]]:
        ...
        Only cameras that can successfully read frames are included in the results.
        This filters out virtual cameras or cameras that are already in use.
        ...
        for target in targets_to_scan:
            # Suppress OpenCV's stderr warnings for out-of-bounds device indices
            # This is expected when scanning indices 0-59 on systems with fewer cameras
            with contextlib.redirect_stderr(io.StringIO()):
                camera = cv2.VideoCapture(target)
            
            if camera.isOpened():
                # Try to read a test frame to verify the camera is actually usable
                # Some cameras can be opened but fail to read frames (e.g., virtual cameras, in-use cameras)
                ret, test_frame = camera.read()
                
                if not ret or test_frame is None:
                    # Camera opened but can't read frames - skip it
                    logger.debug(f"Camera at {target} opened but failed to read frames. Skipping.")
                    camera.release()
                    continue
